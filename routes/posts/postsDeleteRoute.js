...
router.delete('/posts/:postId',verifyAccessToken,postValidation,async(req,res)=>{
    try{
        const postId=req.post._id;
        if(req.user.userId !==req.post.userId.toString()) return res.status(403).json({error:'Unauthorized'})
...
router.delete('/posts/:postId/comments/:commentId',verifyAccessToken,postValidation,async(req,res)=>{
    try{
        const postId=req.post._id;
        ...
        if(postId.toString() !== comment.postId.toString()) return res.status(403).json({error:'Unauthorized'});
        if(req.user.userId.toString() !== comment.userId.toString()) return res.status(403).json({error:'Unathorized'})
        const deletedComment=await commentModel.deleteOne({_id:commentId});
        if(deletedComment.deletedCount===0){
            return res.status(500).json({error:'Failed to delete comment'})
        }
        const updatePostModel=await postModel.findByIdAndUpdate(postId,{$inc:{commentCount:-1}},{new:true})
        if(!updatePostModel){
...
router.delete('/posts/:postId/likes',verifyAccessToken,postValidation,async(req,res)=>{
    try{
        ...
        const hasLiked=await likeModel.findOne({targetId:postId,userId:userId,targetType:'post'});
        if(!hasLiked){
            return res.status(404).json({ error: 'Like not found' });
        }
        const deleteLike=await likeModel.findByIdAndDelete(hasLiked._id);
        if(!deleteLike) return res.status(500).json({error:'Internal server error'})
        const updatedLikeOnPost=await postModel.findByIdAndUpdate(postId,{$inc:{likeCount:-1}},{new:true})
