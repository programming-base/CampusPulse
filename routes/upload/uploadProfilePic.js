...
router.post('/uploads/profile-photo',verifyAccessToken,upload.single('image'),async(req,res)=>{
    ...
    try{
        ...
        Upload=await uploadBufferToCloudinary(req.file.buffer,`campuspulse/profile-photo/${req.user.userId}`)
        const updateProfilePic=await userModel.findByIdAndUpdate(req.user.userId,{$set:{photoURL:{url:Upload.secure_url,publicID:Upload.public_id}}},{new:true,runValidators:true})
        if(!updateProfilePic){
